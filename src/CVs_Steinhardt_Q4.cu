...
double MetaD_zqc::STEIN_Q4::compute_cv(){
    ...
    if (1) {
        double q4_ave=0;
        // my_averager->compute(group_count, stein_q[0], q4_ave);
        my_averager->compute(group_count, d_stein_ql, q4_ave);
        cv_value = q4_ave;
        return cv_value;
    }
}

void MetaD_zqc::STEIN_Q4::cv_method(){
    DEBUG_LOG("MetaD_zqc::STEIN_Q4::cv_method");
}
