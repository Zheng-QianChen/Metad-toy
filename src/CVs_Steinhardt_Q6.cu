...
MetaD_zqc::STEIN_Q6::STEIN_Q6(LAMMPS_NS::LAMMPS *lmp, LAMMPS_NS::FixMetadynamics *Fixmetad, FILE *f_check, int group_id, 
...
double MetaD_zqc::STEIN_Q6::compute_cv(){
    // =======更新一下邻居列表位置防止报错=========
    nlist = Fixmetad->listfull;
    ERR_COND((nlist == nullptr),"STEIN_Q6 CV failed to find neighbor list now.");
...
void MetaD_zqc::STEIN_Q6::bias_force(double dVdcv)
{
    // pass
    DEBUG_LOG("MetaD_zqc::STEIN_Q6::bias_force");
    double **f = lmp->atom->f;
    double **x = lmp->atom->x;
    int c_tag;
...
void MetaD_zqc::STEIN_Q6::get_dcvdx(double cv_value, double *dcvdx)
{
    // DEBUG_RUN(
    delete[] h_stein_qlm;
    ...
    // dcv_steinhardt_param_calc_kernel_q6(
...
void MetaD_zqc::STEIN_Q6::envioronment()
{
    DEBUG_LOG("1");
...
void MetaD_zqc::STEIN_Q6::steinhardt_param_calc(double *stein_ql){
...
void MetaD_zqc::STEIN_Q6::summary(FILE* f){}
