...
#define SAFE_CUDA_MEMCPY(dst, src, size_bytes, kind, file) do { \
        cudaError_t _err = cudaMemcpy((dst), (src), (size_bytes), (kind)); \
        if (_err != cudaSuccess) { \
            fprintf(file, "CUDA Memcpy failed at %s:%d\n", __FILE__, __LINE__); \
            fprintf(file, "  Operation: %s -> %s\n", \
